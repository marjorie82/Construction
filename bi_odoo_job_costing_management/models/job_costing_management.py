...
class JobCostSheet(models.Model):
    ...
    def _get_invoice_line_count(self):
        for job_cost_sheet in self:
            ...
            invoice_ids = self.env['account.move'].search([('job_cost_sheet_id','=',job_cost_sheet.id),('type','=','in_invoice')])
...
class JobCostLine(models.Model):
    ...
    def _compute_invoice_quantity(self):
        for line in self:
            qty = 0
            invoice_line_ids = self.env['account.move.line'].search([('product_id', '=', line.product_id.id),('move_id.state','!=','draft'),('move_id.type','=','in_invoice')])
